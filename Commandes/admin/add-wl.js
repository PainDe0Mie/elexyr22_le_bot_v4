    ...
    async run(bot, message, args, db) {
      ...
      if(message.user ? args._hoistedOptions.length >= 1 : args.length >= 1) {
       ...
       db.query(`SELECT * FROM wl WHERE userID = ${user.id}`, async (err, req) => {
        if(req.length < 1) {           
     let sql = `INSERT INTO wl (userID, username)VALUES ('${user.id}', '${user.username}')`
            db.query(sql, function(err) {
                if(err) throw err})

           return message.reply(` ${user.username} a bien été ajouté a la **Whitelist !** `);
          } else {
            return message.reply(` ${user.username} est déjà ajouté au **Whitelist !** `);
}})}})

...
module.exports = new Command({
    ...
    async run(bot, message, args, db) {
        ...
        message.reply({content: `**Bienvenue sur le panel de création d'embeds de ${bot.user}.**`}).then(async d => {
            let msgembed = await d.reply({embeds: [embed], components: [embedBuilderActionRow, embedBuilderActionRowSendEdit]}).catch(async err => {return;})
            ...
            collectorX.on(`collect`, async (cld) => {
                ...
                if (cld.customId === "embedsend") {
                    var yx = await cld.message.reply({content: "Dans quel salon devrais-je envoyer l'embed ?"})
                    ...
                    .then(async (collected) => {
                        ...
                        if (!channel) return cld.message.reply({content: "Salon introuvable."})

                        channel.send({embeds: [embed]})
                        collected.first().delete()
                        yx.delete()
                        cld.message.reply({content: "L'embed à été envoyé dans le salon **"+channel.name+"**."})
                        cld.message.delete();
                        msgembed.delete();
                        d.delete();
                    })
                } else if (cld.customId === "embededit") {
                    var yx = await cld.message.reply({content: "Dans quel salon est le message à éditer par l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        var channel = cld.message.guild.channels.cache.get(collected.first().content) || collected.first().mentions.channels.first()
                        if (!channel) return cld.message.reply({content: "Salon introuvable."})
                        var yxy = await cld.message.reply({content: "Quel est l'identifiant du message à éditer par l'embed ?"})
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected2) => {
                            var msg = await channel.messages.fetch(collected2.first().content)
                            if (!msg) return cld.message.reply({content: "Message introuvable."})
                            ...
                            yx.delete()
                            yxy.delete()
                            cld.message.reply({content: "Le message à été édité par l'embed dans le salon **"+channel.name+"**."})
                            cld.message.delete();
                            msgembed.delete();
                            d.delete();  
                        })
            ...
            collector.on(`collect`, async (cld) => {
                ...
                if (value === "embedtitle") {
                    var yx = await cld.message.reply({content: "Quel sera le **Titre** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected) => {
                            if (collected.first().content.length > 256) return cld.message.reply("Titre trop long (max 256 caractères).").then(async z => setTimeout(z.delete(), 2000))
                            collected.first().delete();
                            yx.delete();
                    ...
                    var yx = await cld.message.reply({content: "Quel sera la **Description** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected) => {
                            if (collected.first().content.length > 6000) return cld.message.reply({content: "Description trop longue (max 6000 caractères)."}).then(async z => setTimeout(z.delete(), 2000))
                            collected.first().delete();
                            yx.delete();
                    ...
                    var yx = await cld.message.reply({content: "Quel sera la **Couleur** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected) => {
                            if (/^#[0-9A-F]{6}$/i.test(collected.first().content) !== true) return message.reply({content: "Couleur invalide."});
                            collected.first().delete();
                            yx.delete();
                    ...
                    var yx = await cld.message.reply({content: "Quel sera le nom de l'**Auteur** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        if (collected.first().content.length > 64) return cld.message.reply({content: "Nom trop long."}).then(async z => setTimeout(z.delete(), 2000))
                        var yxy = await cld.message.reply({content: "Quel sera l'avatar de l'**Auteur** de l'embed ?"})
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected2) => {
                            var yxx = await cld.message.reply({content: "Quel sera l'url de l'**Auteur** de l'embed ?"})
                            message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    ...
                    var yx = await cld.message.reply({content: "Quel sera le texte du **Footer** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        if (collected.first().content.length > 64) return cld.message.reply({content: "Texte trop long."}).then(async z => setTimeout(z.delete(), 2000))
                        var yxy = await cld.message.reply({content: "Quel sera l'icone du **Footer** de l'embed ?"})
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected2) => {
                            var a;
                    ...
                    var yx = await cld.message.reply({content: "Quel sera le **Thumnail** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        var a;
                        ...
                        if (a === false) {
                            return collected.message.reply({content: "L'image voulue est Invalide."})
                        } else if (a !== false) {
                    ...
                    var yx = await cld.message.reply({content: "Quelle sera l'**Image** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        var a;
                    ...
                    var yx = await cld.message.reply({content: "Quel sera le **Timestamp** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        var a;
                    ...
                    var yx = await cld.message.reply({content: "Quelle sera l'**URL** de l'embed ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        var a;
                        ...
                        if (a === false) {
                            return cld.message.reply({content: "URL invalide."})
                        } else if (a !== false) {
                            embed.setURL(a);
                        }
                        msgembed.edit({ embeds: [embed] })
                    ...
                    var yx = await cld.message.reply({content: "Quel sera le nom du **Field** ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        if (collected.first().content.length > 128) return cld.message.reply({content: "Nom trop long."})
                        var yxy = await cld.message.reply({content: "Quel sera la description du **Field** ?"})
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected2) => {
                            if (embed.fields.length === 25) return cld.message.reply({content: "Il y a trop de fields sur cet embed."})
                            collected.first().delete();
                            collected2.first().delete();
                            yx.delete();
                            yxy.delete();
                    ...
                    var yx = await cld.message.reply({content: "Quel est le numéro du **Field** ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {       
                        if (embed.fields.length < 1) return cld.message.reply({content: "Aucun field trouvé sur l'embed."})
                        if (isNaN(collected.first().content)) return cld.message.reply({content: "La valeur spécifiée doit être un numéro."})
                        if (collected.first().content > embed.fields.length) return cld.message.reply({content: "Le numéro est trop élevé."})
                        var indexField = Number(collected.first().content) - 1
                        embed.spliceFields(indexField, 1)
                        msgembed.edit({ embeds: [embed] })
                    })
                } else if (value === "embedcopyother") {
                    var yx = await cld.message.reply({content: "Quel est le salon de l'**Embed** ?"})
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                    .then(async (collected) => {
                        var channel = cld.message.guild.channels.cache.get(collected.first().content) || collected.first().mentions.channels.first()
                        if (!channel) return cld.message.reply({content: "Salon introuvable."})
                        var yxy = await cld.message.reply({content: "Quel est le message de l'**Embed** ?"})
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected2) => {
                            var messag = await channel.messages.fetch(collected2.first().content)
                            if (!messag) return cld.message.reply({content: "Message introuvable."})
                            collected.first().delete();
                            collected2.first().delete();
                            yx.delete();
                            yxy.delete();
                            embed = new MessageEmbed({description: "\u200B"})
                            if (!messag.embeds) return cld.message.reply({content: "Aucun embed trouvé dans le message spécifié."});
                            if (messag.embeds.length < 1) return cld.message.reply({content: "Aucun embed trouvé dans le message spécifié."});

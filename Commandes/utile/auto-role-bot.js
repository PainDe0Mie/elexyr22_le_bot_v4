...
module.exports = new Command({
    ...
    name: "auto-role-bot",
    description: "Permet d'avoir un auto-rôle sûr les bots",
    utilisation: "",
    alias: ["auto-role-bot", "bot"],
    ...
    async run(bot, message, args, db) {
        try {
            ...
            if (!roleID || isNaN(roleID)) {
                ...
                sql = `UPDATE serveur SET roleBOT = '${roleID}' WHERE guildID = ${message.guild.id}`;
                db.query(sql, function (err) {
                    if (err) throw err;
                });

                message.reply(`<:elexyr22:1067501213085597806> **L'auto-Rôle-Bot** est bien __activé__ sûr ${role} !`);
            }
        } catch (err) {
            console.log(err);
        }
